import {
  EducationSystem,
  EducationLevel,
  ClassLevel,
} from '../interfaces/education-system.interface';

// Nigerian Education System
export const nigerianSystem: EducationSystem = {
  id: 'nigeria-6334',
  countryCode: 'NG',
  systemName: 'Nigerian Education System',
  description:
    '6-3-3-4 system: 6 years Primary, 3 years JSS, 3 years SSS, 4 years University',
  flag: '🇳🇬',
  phoneCode: '+234',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 9, // September
    academicYearEnd: 7, // July
    vacationPeriods: [
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'End of year holiday break',
      },
      {
        name: 'Easter Break',
        startMonth: 3,
        endMonth: 4,
        description: 'Easter holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 7,
        endMonth: 8,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'nursery',
      name: 'Nursery School',
      description: '2 years of nursery education',
      ageRange: [3, 5],
      order: 1,
      classLevels: [
        {
          id: 'nursery1',
          name: 'Nursery 1',
          ageRange: [3, 4],
          description: 'First year of nursery',
          order: 1,
          subjects: [],
        },
        {
          id: 'nursery2',
          name: 'Nursery 2',
          ageRange: [4, 5],
          description: 'Second year of nursery',
          order: 2,
          subjects: [],
        },
      ],
      subjects: ['Basic Skills', 'Social Development'],
    },
    {
      id: 'primary',
      name: 'Primary School',
      description: '6 years of primary education',
      ageRange: [5, 11],
      order: 2,
      classLevels: [
        {
          id: 'primary1',
          name: 'Primary 1',
          ageRange: [5, 6],
          description: 'First year of primary',
          order: 1,
          subjects: [],
        },
        {
          id: 'primary2',
          name: 'Primary 2',
          ageRange: [6, 7],
          description: 'Second year of primary',
          order: 2,
          subjects: [],
        },
        {
          id: 'primary3',
          name: 'Primary 3',
          ageRange: [7, 8],
          description: 'Third year of primary',
          order: 3,
          subjects: [],
        },
        {
          id: 'primary4',
          name: 'Primary 4',
          ageRange: [8, 9],
          description: 'Fourth year of primary',
          order: 4,
          subjects: [],
        },
        {
          id: 'primary5',
          name: 'Primary 5',
          ageRange: [9, 10],
          description: 'Fifth year of primary',
          order: 5,
          subjects: [],
        },
        {
          id: 'primary6',
          name: 'Primary 6',
          ageRange: [10, 11],
          description: 'Sixth year of primary',
          order: 6,
          subjects: [],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Social Studies',
        'Basic Science',
        'Religious Studies',
        'Creative Arts',
      ],
      nationalExam: 'Primary School Leaving Certificate',
    },
    {
      id: 'jss',
      name: 'Junior Secondary School (JSS)',
      description: '3 years of junior secondary education',
      ageRange: [11, 14],
      order: 3,
      classLevels: [
        {
          id: 'jss1',
          name: 'JSS 1',
          ageRange: [11, 12],
          description: 'First year of JSS',
          order: 1,
          subjects: [],
        },
        {
          id: 'jss2',
          name: 'JSS 2',
          ageRange: [12, 13],
          description: 'Second year of JSS',
          order: 2,
          subjects: [],
        },
        {
          id: 'jss3',
          name: 'JSS 3',
          ageRange: [13, 14],
          description: 'Third year of JSS',
          order: 3,
          subjects: [],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Basic Science',
        'Social Studies',
        'Agricultural Science',
        'Home Economics',
        'Business Studies',
      ],
      nationalExam: 'Junior Secondary School Certificate (JSSC)',
    },
    {
      id: 'sss',
      name: 'Senior Secondary School (SSS)',
      description: '3 years of senior secondary education',
      ageRange: [14, 17],
      order: 4,
      classLevels: [
        {
          id: 'sss1',
          name: 'SSS 1',
          ageRange: [14, 15],
          description: 'First year of SSS',
          order: 1,
          subjects: [],
        },
        {
          id: 'sss2',
          name: 'SSS 2',
          ageRange: [15, 16],
          description: 'Second year of SSS',
          order: 2,
          subjects: [],
        },
        {
          id: 'sss3',
          name: 'SSS 3',
          ageRange: [16, 17],
          description: 'Third year of SSS',
          order: 3,
          subjects: [],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'Economics',
        'Government',
        'Literature',
        'History',
        'Geography',
      ],
      nationalExam: 'Senior Secondary School Certificate (SSCE)',
    },
  ],
};

// Ghanaian Education System
export const ghanaianSystem: EducationSystem = {
  id: 'ghana-2634',
  countryCode: 'GH',
  systemName: 'Ghanaian Education System',
  description:
    '2-6-3-4 system: 2 years Kindergarten, 6 years Primary, 3 years JHS, 3 years SHS, 4 years University',
  flag: '🇬🇭',
  phoneCode: '+233',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 9, // September
    academicYearEnd: 7, // July
    vacationPeriods: [
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'End of year holiday break',
      },
      {
        name: 'Easter Break',
        startMonth: 3,
        endMonth: 4,
        description: 'Easter holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 7,
        endMonth: 8,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'kindergarten',
      name: 'Kindergarten',
      description: '2 years of kindergarten education',
      ageRange: [4, 6],
      order: 1,
      classLevels: [
        {
          id: 'kg1',
          name: 'KG 1',
          ageRange: [4, 5],
          description: 'First year of kindergarten',
          order: 1,
          subjects: ['Basic Skills', 'Social Development'],
        },
        {
          id: 'kg2',
          name: 'KG 2',
          ageRange: [5, 6],
          description: 'Second year of kindergarten',
          order: 2,
          subjects: ['Basic Skills', 'Social Development'],
        },
      ],
      subjects: ['Basic Skills', 'Social Development'],
    },
    {
      id: 'primary',
      name: 'Primary School',
      description: '6 years of primary education',
      ageRange: [6, 12],
      order: 2,
      classLevels: [
        {
          id: 'primary1',
          name: 'Primary 1',
          ageRange: [6, 7],
          description: 'First year of primary',
          order: 1,
          subjects: [
            'English',
            'Mathematics',
            'Social Studies',
            'Integrated Science',
          ],
        },
        {
          id: 'primary2',
          name: 'Primary 2',
          ageRange: [7, 8],
          description: 'Second year of primary',
          order: 2,
          subjects: [
            'English',
            'Mathematics',
            'Social Studies',
            'Integrated Science',
          ],
        },
        {
          id: 'primary3',
          name: 'Primary 3',
          ageRange: [8, 9],
          description: 'Third year of primary',
          order: 3,
          subjects: [
            'English',
            'Mathematics',
            'Social Studies',
            'Integrated Science',
          ],
        },
        {
          id: 'primary4',
          name: 'Primary 4',
          ageRange: [9, 10],
          description: 'Fourth year of primary',
          order: 4,
          subjects: [
            'English',
            'Mathematics',
            'Social Studies',
            'Integrated Science',
          ],
        },
        {
          id: 'primary5',
          name: 'Primary 5',
          ageRange: [10, 11],
          description: 'Fifth year of primary',
          order: 5,
          subjects: [
            'English',
            'Mathematics',
            'Social Studies',
            'Integrated Science',
          ],
        },
        {
          id: 'primary6',
          name: 'Primary 6',
          ageRange: [11, 12],
          description: 'Sixth year of primary',
          order: 6,
          subjects: [
            'English',
            'Mathematics',
            'Social Studies',
            'Integrated Science',
          ],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Social Studies',
        'Integrated Science',
        'Religious and Moral Education',
        'Creative Arts',
      ],
      nationalExam: 'Primary School Leaving Certificate',
    },
    {
      id: 'jhs',
      name: 'Junior High School (JHS)',
      description: '3 years of junior high school education',
      ageRange: [12, 15],
      order: 3,
      classLevels: [
        {
          id: 'jhs1',
          name: 'JHS 1',
          ageRange: [12, 13],
          description: 'First year of JHS',
          order: 1,
          subjects: [
            'English',
            'Mathematics',
            'Integrated Science',
            'Social Studies',
          ],
        },
        {
          id: 'jhs2',
          name: 'JHS 2',
          ageRange: [13, 14],
          description: 'Second year of JHS',
          order: 2,
          subjects: [
            'English',
            'Mathematics',
            'Integrated Science',
            'Social Studies',
          ],
        },
        {
          id: 'jhs3',
          name: 'JHS 3',
          ageRange: [14, 15],
          description: 'Third year of JHS',
          order: 3,
          subjects: [
            'English',
            'Mathematics',
            'Integrated Science',
            'Social Studies',
          ],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Integrated Science',
        'Social Studies',
        'Agricultural Science',
        'Home Economics',
        'Business Studies',
      ],
      nationalExam: 'Basic Education Certificate Examination (BECE)',
    },
    {
      id: 'shs',
      name: 'Senior High School (SHS)',
      description: '3 years of senior high school education',
      ageRange: [15, 18],
      order: 4,
      classLevels: [
        {
          id: 'shs1',
          name: 'SHS 1',
          ageRange: [15, 16],
          description: 'First year of SHS',
          order: 1,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'shs2',
          name: 'SHS 2',
          ageRange: [16, 17],
          description: 'Second year of SHS',
          order: 2,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'shs3',
          name: 'SHS 3',
          ageRange: [17, 18],
          description: 'Third year of SHS',
          order: 3,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'Economics',
        'Government',
        'Literature',
        'History',
        'Geography',
      ],
      nationalExam:
        'West African Senior School Certificate Examination (WASSCE)',
    },
  ],
};

// Kenyan Education System
export const kenyanSystem: EducationSystem = {
  id: 'kenya-844',
  countryCode: 'KE',
  systemName: 'Kenyan Education System',
  description:
    '8-4-4 system: 8 years Primary, 4 years Secondary, 4 years University',
  flag: '🇰🇪',
  phoneCode: '+254',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 12,
  },
  calendar: {
    academicYearStart: 1, // January
    academicYearEnd: 12, // December
    vacationPeriods: [
      {
        name: 'April Holiday',
        startMonth: 4,
        endMonth: 4,
        description: 'Mid-term break',
      },
      {
        name: 'August Holiday',
        startMonth: 8,
        endMonth: 8,
        description: 'Mid-term break',
      },
      {
        name: 'December Holiday',
        startMonth: 12,
        endMonth: 1,
        description: 'End of year holiday break',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'Primary School',
      description: '8 years of primary education',
      ageRange: [6, 14],
      order: 1,
      classLevels: [
        {
          id: 'std1',
          name: 'Standard 1',
          ageRange: [6, 7],
          description: 'First year of primary',
          order: 1,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Environmental Activities',
          ],
        },
        {
          id: 'std2',
          name: 'Standard 2',
          ageRange: [7, 8],
          description: 'Second year of primary',
          order: 2,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Environmental Activities',
          ],
        },
        {
          id: 'std3',
          name: 'Standard 3',
          ageRange: [8, 9],
          description: 'Third year of primary',
          order: 3,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Environmental Activities',
          ],
        },
        {
          id: 'std4',
          name: 'Standard 4',
          ageRange: [9, 10],
          description: 'Fourth year of primary',
          order: 4,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Environmental Activities',
          ],
        },
        {
          id: 'std5',
          name: 'Standard 5',
          ageRange: [10, 11],
          description: 'Fifth year of primary',
          order: 5,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Science and Technology',
            'Social Studies',
          ],
        },
        {
          id: 'std6',
          name: 'Standard 6',
          ageRange: [11, 12],
          description: 'Sixth year of primary',
          order: 6,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Science and Technology',
            'Social Studies',
          ],
        },
        {
          id: 'std7',
          name: 'Standard 7',
          ageRange: [12, 13],
          description: 'Seventh year of primary',
          order: 7,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Science and Technology',
            'Social Studies',
          ],
        },
        {
          id: 'std8',
          name: 'Standard 8',
          ageRange: [13, 14],
          description: 'Eighth year of primary',
          order: 8,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Science and Technology',
            'Social Studies',
          ],
        },
      ],
      subjects: [
        'English',
        'Kiswahili',
        'Mathematics',
        'Science and Technology',
        'Social Studies',
        'Religious Education',
        'Creative Arts',
      ],
      nationalExam: 'Kenya Certificate of Primary Education (KCPE)',
    },
    {
      id: 'secondary',
      name: 'Secondary School',
      description: '4 years of secondary education',
      ageRange: [14, 18],
      order: 2,
      classLevels: [
        {
          id: 'form1',
          name: 'Form 1',
          ageRange: [14, 15],
          description: 'First year of secondary',
          order: 1,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form2',
          name: 'Form 2',
          ageRange: [15, 16],
          description: 'Second year of secondary',
          order: 2,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form3',
          name: 'Form 3',
          ageRange: [16, 17],
          description: 'Third year of secondary',
          order: 3,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form4',
          name: 'Form 4',
          ageRange: [17, 18],
          description: 'Fourth year of secondary',
          order: 4,
          subjects: [
            'English',
            'Kiswahili',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'English',
        'Kiswahili',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'History',
        'Geography',
        'Religious Education',
        'Business Studies',
      ],
      nationalExam: 'Kenya Certificate of Secondary Education (KCSE)',
    },
  ],
};

// Ugandan Education System
export const ugandanSystem: EducationSystem = {
  id: 'uganda-7423',
  countryCode: 'UG',
  systemName: 'Ugandan Education System',
  description:
    '7-4-2-3 system: 7 years Primary, 4 years O-Level, 2 years A-Level, 3+ years University',
  flag: '🇺🇬',
  phoneCode: '+256',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 2, // February
    academicYearEnd: 12, // December
    vacationPeriods: [
      {
        name: 'April Holiday',
        startMonth: 4,
        endMonth: 4,
        description: 'Mid-term break',
      },
      {
        name: 'August Holiday',
        startMonth: 8,
        endMonth: 8,
        description: 'Mid-term break',
      },
      {
        name: 'December Holiday',
        startMonth: 12,
        endMonth: 1,
        description: 'End of year holiday break',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'Primary School',
      description: '7 years of primary education',
      ageRange: [6, 13],
      order: 1,
      classLevels: [
        {
          id: 'p1',
          name: 'Primary 1',
          ageRange: [6, 7],
          description: 'First year of primary',
          order: 1,
          subjects: ['English', 'Mathematics', 'Social Studies', 'Science'],
        },
        {
          id: 'p2',
          name: 'Primary 2',
          ageRange: [7, 8],
          description: 'Second year of primary',
          order: 2,
          subjects: ['English', 'Mathematics', 'Social Studies', 'Science'],
        },
        {
          id: 'p3',
          name: 'Primary 3',
          ageRange: [8, 9],
          description: 'Third year of primary',
          order: 3,
          subjects: ['English', 'Mathematics', 'Social Studies', 'Science'],
        },
        {
          id: 'p4',
          name: 'Primary 4',
          ageRange: [9, 10],
          description: 'Fourth year of primary',
          order: 4,
          subjects: ['English', 'Mathematics', 'Social Studies', 'Science'],
        },
        {
          id: 'p5',
          name: 'Primary 5',
          ageRange: [10, 11],
          description: 'Fifth year of primary',
          order: 5,
          subjects: ['English', 'Mathematics', 'Social Studies', 'Science'],
        },
        {
          id: 'p6',
          name: 'Primary 6',
          ageRange: [11, 12],
          description: 'Sixth year of primary',
          order: 6,
          subjects: ['English', 'Mathematics', 'Social Studies', 'Science'],
        },
        {
          id: 'p7',
          name: 'Primary 7',
          ageRange: [12, 13],
          description: 'Seventh year of primary',
          order: 7,
          subjects: ['English', 'Mathematics', 'Social Studies', 'Science'],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Social Studies',
        'Science',
        'Religious Education',
        'Creative Arts',
      ],
      nationalExam: 'Primary Leaving Examination (PLE)',
    },
    {
      id: 'olevel',
      name: 'O-Level (Ordinary Level)',
      description: '4 years of O-Level education',
      ageRange: [13, 17],
      order: 2,
      classLevels: [
        {
          id: 's1',
          name: 'Senior 1',
          ageRange: [13, 14],
          description: 'First year of O-Level',
          order: 1,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 's2',
          name: 'Senior 2',
          ageRange: [14, 15],
          description: 'Second year of O-Level',
          order: 2,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 's3',
          name: 'Senior 3',
          ageRange: [15, 16],
          description: 'Third year of O-Level',
          order: 3,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 's4',
          name: 'Senior 4',
          ageRange: [16, 17],
          description: 'Fourth year of O-Level',
          order: 4,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'History',
        'Geography',
        'Economics',
        'Literature',
      ],
      nationalExam: 'Uganda Certificate of Education (UCE)',
    },
    {
      id: 'alevel',
      name: 'A-Level (Advanced Level)',
      description: '2 years of A-Level education',
      ageRange: [17, 19],
      order: 3,
      classLevels: [
        {
          id: 's5',
          name: 'Senior 5',
          ageRange: [17, 18],
          description: 'First year of A-Level',
          order: 1,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 's6',
          name: 'Senior 6',
          ageRange: [18, 19],
          description: 'Second year of A-Level',
          order: 2,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'History',
        'Geography',
        'Economics',
        'Literature',
      ],
      nationalExam: 'Uganda Advanced Certificate of Education (UACE)',
    },
  ],
};

// Tanzanian Education System
export const tanzanianSystem: EducationSystem = {
  id: 'tanzania-7423',
  countryCode: 'TZ',
  systemName: 'Tanzanian Education System',
  description:
    '7-4-2-3 system: 7 years Primary, 4 years O-Level, 2 years A-Level, 3+ years University',
  flag: '🇹🇿',
  phoneCode: '+255',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 1, // January
    academicYearEnd: 12, // December
    vacationPeriods: [
      {
        name: 'April Holiday',
        startMonth: 4,
        endMonth: 4,
        description: 'Mid-term break',
      },
      {
        name: 'August Holiday',
        startMonth: 8,
        endMonth: 8,
        description: 'Mid-term break',
      },
      {
        name: 'December Holiday',
        startMonth: 12,
        endMonth: 1,
        description: 'End of year holiday break',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'Primary School',
      description: '7 years of primary education',
      ageRange: [6, 13],
      order: 1,
      classLevels: [
        {
          id: 'std1',
          name: 'Standard 1',
          ageRange: [6, 7],
          description: 'First year of primary',
          order: 1,
          subjects: ['Kiswahili', 'English', 'Mathematics', 'Science'],
        },
        {
          id: 'std2',
          name: 'Standard 2',
          ageRange: [7, 8],
          description: 'Second year of primary',
          order: 2,
          subjects: ['Kiswahili', 'English', 'Mathematics', 'Science'],
        },
        {
          id: 'std3',
          name: 'Standard 3',
          ageRange: [8, 9],
          description: 'Third year of primary',
          order: 3,
          subjects: ['Kiswahili', 'English', 'Mathematics', 'Science'],
        },
        {
          id: 'std4',
          name: 'Standard 4',
          ageRange: [9, 10],
          description: 'Fourth year of primary',
          order: 4,
          subjects: ['Kiswahili', 'English', 'Mathematics', 'Science'],
        },
        {
          id: 'std5',
          name: 'Standard 5',
          ageRange: [10, 11],
          description: 'Fifth year of primary',
          order: 5,
          subjects: ['Kiswahili', 'English', 'Mathematics', 'Science'],
        },
        {
          id: 'std6',
          name: 'Standard 6',
          ageRange: [11, 12],
          description: 'Sixth year of primary',
          order: 6,
          subjects: ['Kiswahili', 'English', 'Mathematics', 'Science'],
        },
        {
          id: 'std7',
          name: 'Standard 7',
          ageRange: [12, 13],
          description: 'Seventh year of primary',
          order: 7,
          subjects: ['Kiswahili', 'English', 'Mathematics', 'Science'],
        },
      ],
      subjects: [
        'Kiswahili',
        'English',
        'Mathematics',
        'Science',
        'Social Studies',
        'Religious Education',
        'Creative Arts',
      ],
      nationalExam: 'Primary School Leaving Examination (PSLE)',
    },
    {
      id: 'olevel',
      name: 'O-Level (Ordinary Level)',
      description: '4 years of O-Level education',
      ageRange: [13, 17],
      order: 2,
      classLevels: [
        {
          id: 'form1',
          name: 'Form 1',
          ageRange: [13, 14],
          description: 'First year of O-Level',
          order: 1,
          subjects: [
            'Kiswahili',
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form2',
          name: 'Form 2',
          ageRange: [14, 15],
          description: 'Second year of O-Level',
          order: 2,
          subjects: [
            'Kiswahili',
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form3',
          name: 'Form 3',
          ageRange: [15, 16],
          description: 'Third year of O-Level',
          order: 3,
          subjects: [
            'Kiswahili',
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form4',
          name: 'Form 4',
          ageRange: [16, 17],
          description: 'Fourth year of O-Level',
          order: 4,
          subjects: [
            'Kiswahili',
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'Kiswahili',
        'English',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'History',
        'Geography',
        'Economics',
      ],
      nationalExam: 'Certificate of Secondary Education (CSE)',
    },
    {
      id: 'alevel',
      name: 'A-Level (Advanced Level)',
      description: '2 years of A-Level education',
      ageRange: [17, 19],
      order: 3,
      classLevels: [
        {
          id: 'form5',
          name: 'Form 5',
          ageRange: [17, 18],
          description: 'First year of A-Level',
          order: 1,
          subjects: [
            'General Studies',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form6',
          name: 'Form 6',
          ageRange: [18, 19],
          description: 'Second year of A-Level',
          order: 2,
          subjects: [
            'General Studies',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'General Studies',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'History',
        'Geography',
        'Economics',
        'Literature',
      ],
      nationalExam: 'Advanced Certificate of Secondary Education (ACSE)',
    },
  ],
};

// South African Education System
export const southAfricanSystem: EducationSystem = {
  id: 'south-africa-1234',
  countryCode: 'ZA',
  systemName: 'South African Education System',
  description:
    '1-2-3-4 system: 1 year Grade R, 2 years Foundation Phase, 3 years Intermediate Phase, 4 years Senior Phase',
  flag: '🇿🇦',
  phoneCode: '+27',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 12,
  },
  calendar: {
    academicYearStart: 1, // January
    academicYearEnd: 12, // December
    vacationPeriods: [
      {
        name: 'March Holiday',
        startMonth: 3,
        endMonth: 3,
        description: 'Mid-term break',
      },
      {
        name: 'June Holiday',
        startMonth: 6,
        endMonth: 6,
        description: 'Mid-term break',
      },
      {
        name: 'September Holiday',
        startMonth: 9,
        endMonth: 9,
        description: 'Mid-term break',
      },
      {
        name: 'December Holiday',
        startMonth: 12,
        endMonth: 1,
        description: 'End of year holiday break',
      },
    ],
  },
  levels: [
    {
      id: 'grade-r',
      name: 'Grade R',
      description: '1 year of Grade R (Reception)',
      ageRange: [5, 6],
      order: 1,
      classLevels: [
        {
          id: 'grade-r',
          name: 'Grade R',
          ageRange: [5, 6],
          description: 'Reception year',
          order: 1,
          subjects: ['Life Skills', 'Language', 'Mathematics'],
        },
      ],
      subjects: ['Life Skills', 'Language', 'Mathematics'],
    },
    {
      id: 'foundation',
      name: 'Foundation Phase',
      description: '3 years of foundation phase (Grades 1-3)',
      ageRange: [6, 9],
      order: 2,
      classLevels: [
        {
          id: 'grade1',
          name: 'Grade 1',
          ageRange: [6, 7],
          description: 'First grade',
          order: 1,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Life Skills',
          ],
        },
        {
          id: 'grade2',
          name: 'Grade 2',
          ageRange: [7, 8],
          description: 'Second grade',
          order: 2,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Life Skills',
          ],
        },
        {
          id: 'grade3',
          name: 'Grade 3',
          ageRange: [8, 9],
          description: 'Third grade',
          order: 3,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Life Skills',
          ],
        },
      ],
      subjects: [
        'Home Language',
        'First Additional Language',
        'Mathematics',
        'Life Skills',
      ],
    },
    {
      id: 'intermediate',
      name: 'Intermediate Phase',
      description: '3 years of intermediate phase (Grades 4-6)',
      ageRange: [9, 12],
      order: 3,
      classLevels: [
        {
          id: 'grade4',
          name: 'Grade 4',
          ageRange: [9, 10],
          description: 'Fourth grade',
          order: 1,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Natural Sciences',
            'Social Sciences',
            'Life Skills',
          ],
        },
        {
          id: 'grade5',
          name: 'Grade 5',
          ageRange: [10, 11],
          description: 'Fifth grade',
          order: 2,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Natural Sciences',
            'Social Sciences',
            'Life Skills',
          ],
        },
        {
          id: 'grade6',
          name: 'Grade 6',
          ageRange: [11, 12],
          description: 'Sixth grade',
          order: 3,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Natural Sciences',
            'Social Sciences',
            'Life Skills',
          ],
        },
      ],
      subjects: [
        'Home Language',
        'First Additional Language',
        'Mathematics',
        'Natural Sciences',
        'Social Sciences',
        'Life Skills',
      ],
    },
    {
      id: 'senior',
      name: 'Senior Phase',
      description: '3 years of senior phase (Grades 7-9)',
      ageRange: [12, 15],
      order: 4,
      classLevels: [
        {
          id: 'grade7',
          name: 'Grade 7',
          ageRange: [12, 13],
          description: 'Seventh grade',
          order: 1,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Natural Sciences',
            'Social Sciences',
            'Life Orientation',
            'Creative Arts',
            'Economic Management Sciences',
          ],
        },
        {
          id: 'grade8',
          name: 'Grade 8',
          ageRange: [13, 14],
          description: 'Eighth grade',
          order: 2,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Natural Sciences',
            'Social Sciences',
            'Life Orientation',
            'Creative Arts',
            'Economic Management Sciences',
          ],
        },
        {
          id: 'grade9',
          name: 'Grade 9',
          ageRange: [14, 15],
          description: 'Ninth grade',
          order: 3,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Natural Sciences',
            'Social Sciences',
            'Life Orientation',
            'Creative Arts',
            'Economic Management Sciences',
          ],
        },
      ],
      subjects: [
        'Home Language',
        'First Additional Language',
        'Mathematics',
        'Natural Sciences',
        'Social Sciences',
        'Life Orientation',
        'Creative Arts',
        'Economic Management Sciences',
      ],
      nationalExam: 'General Education and Training Certificate (GETC)',
    },
    {
      id: 'further',
      name: 'Further Education and Training (FET)',
      description: '3 years of FET phase (Grades 10-12)',
      ageRange: [15, 18],
      order: 5,
      classLevels: [
        {
          id: 'grade10',
          name: 'Grade 10',
          ageRange: [15, 16],
          description: 'Tenth grade',
          order: 1,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Life Orientation',
            'Elective Subjects',
          ],
        },
        {
          id: 'grade11',
          name: 'Grade 11',
          ageRange: [16, 17],
          description: 'Eleventh grade',
          order: 2,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Life Orientation',
            'Elective Subjects',
          ],
        },
        {
          id: 'grade12',
          name: 'Grade 12',
          ageRange: [17, 18],
          description: 'Twelfth grade',
          order: 3,
          subjects: [
            'Home Language',
            'First Additional Language',
            'Mathematics',
            'Life Orientation',
            'Elective Subjects',
          ],
        },
      ],
      subjects: [
        'Home Language',
        'First Additional Language',
        'Mathematics',
        'Life Orientation',
        'Elective Subjects',
      ],
      nationalExam: 'National Senior Certificate (NSC)',
    },
  ],
};

// Zimbabwean Education System
export const zimbabweanSystem: EducationSystem = {
  id: 'zimbabwe',
  countryCode: 'ZW',
  systemName: 'Zimbabwean Education System',
  description:
    '7-4-2-3 system: 7 years Primary, 4 years O-Level, 2 years A-Level, 3+ years University',
  flag: '🇿🇼',
  phoneCode: '+263',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 1, // January
    academicYearEnd: 11, // November
    vacationPeriods: [
      {
        name: 'April Break',
        startMonth: 4,
        endMonth: 4,
        description: 'Short April holiday',
      },
      {
        name: 'August Break',
        startMonth: 8,
        endMonth: 8,
        description: 'August holiday break',
      },
      {
        name: 'December Break',
        startMonth: 12,
        endMonth: 1,
        description: 'Long December vacation',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'Primary School',
      description: '7 years of primary education',
      ageRange: [6, 12],
      order: 1,
      classLevels: [
        {
          id: 'grade1',
          name: 'Grade 1',
          ageRange: [6, 7],
          description: 'First year of primary',
          order: 1,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Environmental Science',
          ],
        },
        {
          id: 'grade2',
          name: 'Grade 2',
          ageRange: [7, 8],
          description: 'Second year of primary',
          order: 2,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Environmental Science',
          ],
        },
        {
          id: 'grade3',
          name: 'Grade 3',
          ageRange: [8, 9],
          description: 'Third year of primary',
          order: 3,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Environmental Science',
          ],
        },
        {
          id: 'grade4',
          name: 'Grade 4',
          ageRange: [9, 10],
          description: 'Fourth year of primary',
          order: 4,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Environmental Science',
          ],
        },
        {
          id: 'grade5',
          name: 'Grade 5',
          ageRange: [10, 11],
          description: 'Fifth year of primary',
          order: 5,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Environmental Science',
          ],
        },
        {
          id: 'grade6',
          name: 'Grade 6',
          ageRange: [11, 12],
          description: 'Sixth year of primary',
          order: 6,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Environmental Science',
          ],
        },
        {
          id: 'grade7',
          name: 'Grade 7',
          ageRange: [12, 13],
          description: 'Seventh year of primary',
          order: 7,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Environmental Science',
          ],
        },
      ],
      subjects: [
        'English',
        'Shona/Ndebele',
        'Mathematics',
        'Environmental Science',
        'Religious Education',
        'Creative Arts',
      ],
      nationalExam: 'Primary School Leaving Certificate',
    },
    {
      id: 'olevel',
      name: 'O-Level (Ordinary Level)',
      description: '4 years of O-Level education',
      ageRange: [13, 16],
      order: 2,
      classLevels: [
        {
          id: 'form1',
          name: 'Form 1',
          ageRange: [13, 14],
          description: 'First year of O-Level',
          order: 1,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form2',
          name: 'Form 2',
          ageRange: [14, 15],
          description: 'Second year of O-Level',
          order: 2,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form3',
          name: 'Form 3',
          ageRange: [15, 16],
          description: 'Third year of O-Level',
          order: 3,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form4',
          name: 'Form 4',
          ageRange: [16, 17],
          description: 'Fourth year of O-Level',
          order: 4,
          subjects: [
            'English',
            'Shona/Ndebele',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'English',
        'Shona/Ndebele',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'History',
        'Geography',
        'Economics',
      ],
      nationalExam: 'Zimbabwe Junior Certificate (ZJC)',
    },
    {
      id: 'alevel',
      name: 'A-Level (Advanced Level)',
      description: '2 years of A-Level education',
      ageRange: [17, 18],
      order: 3,
      classLevels: [
        {
          id: 'form5',
          name: 'Form 5',
          ageRange: [17, 18],
          description: 'First year of A-Level',
          order: 1,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'form6',
          name: 'Form 6',
          ageRange: [18, 19],
          description: 'Second year of A-Level',
          order: 2,
          subjects: [
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'History',
        'Geography',
        'Economics',
      ],
      nationalExam: 'Zimbabwe General Certificate of Education (ZGCE)',
    },
  ],
};

// Senegalese Education System
export const senegaleseSystem: EducationSystem = {
  id: 'senegal',
  countryCode: 'SN',
  systemName: 'Senegalese Education System',
  description:
    '6-4-3 system: 6 years Primary, 4 years Collège, 3 years Lycée, 3+ years University',
  flag: '🇸🇳',
  phoneCode: '+221',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 10, // October
    academicYearEnd: 6, // June
    vacationPeriods: [
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'Christmas holiday break',
      },
      {
        name: 'Easter Break',
        startMonth: 3,
        endMonth: 4,
        description: 'Easter holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 7,
        endMonth: 9,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'École Primaire',
      description: '6 years of primary education',
      ageRange: [6, 11],
      order: 1,
      classLevels: [
        {
          id: 'cp1',
          name: 'CP 1',
          ageRange: [6, 7],
          description: 'Cours Préparatoire 1',
          order: 1,
          subjects: ['Français', 'Mathématiques', 'Éveil'],
        },
        {
          id: 'cp2',
          name: 'CP 2',
          ageRange: [7, 8],
          description: 'Cours Préparatoire 2',
          order: 2,
          subjects: ['Français', 'Mathématiques', 'Éveil'],
        },
        {
          id: 'ce1',
          name: 'CE 1',
          ageRange: [8, 9],
          description: 'Cours Élémentaire 1',
          order: 3,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'ce2',
          name: 'CE 2',
          ageRange: [9, 10],
          description: 'Cours Élémentaire 2',
          order: 4,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'cm1',
          name: 'CM 1',
          ageRange: [10, 11],
          description: 'Cours Moyen 1',
          order: 5,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'cm2',
          name: 'CM 2',
          ageRange: [11, 12],
          description: 'Cours Moyen 2',
          order: 6,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
      ],
      subjects: [
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Éducation Civique',
      ],
      nationalExam: "Certificat de Fin d'Études Élémentaires (CFEE)",
    },
    {
      id: 'college',
      name: 'Collège',
      description: '4 years of collège education',
      ageRange: [12, 15],
      order: 2,
      classLevels: [
        {
          id: 'sixieme',
          name: '6ème',
          ageRange: [12, 13],
          description: 'Sixième année',
          order: 1,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'cinquieme',
          name: '5ème',
          ageRange: [13, 14],
          description: 'Cinquième année',
          order: 2,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'quatrieme',
          name: '4ème',
          ageRange: [14, 15],
          description: 'Quatrième année',
          order: 3,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'troisieme',
          name: '3ème',
          ageRange: [15, 16],
          description: 'Troisième année',
          order: 4,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
      ],
      subjects: [
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Anglais',
        'Espagnol',
        'Allemand',
      ],
      nationalExam: "Brevet de Fin d'Études Moyennes (BFEM)",
    },
    {
      id: 'lycee',
      name: 'Lycée',
      description: '3 years of lycée education',
      ageRange: [16, 18],
      order: 3,
      classLevels: [
        {
          id: 'seconde',
          name: 'Seconde',
          ageRange: [16, 17],
          description: 'Seconde année',
          order: 1,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'premiere',
          name: 'Première',
          ageRange: [17, 18],
          description: 'Première année',
          order: 2,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'terminale',
          name: 'Terminale',
          ageRange: [18, 19],
          description: 'Terminale année',
          order: 3,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
      ],
      subjects: [
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Anglais',
        'Philosophie',
      ],
      nationalExam: 'Baccalauréat',
    },
  ],
};

// Ivorian Education System
export const ivorianSystem: EducationSystem = {
  id: 'cote_divoire',
  countryCode: 'CI',
  systemName: 'Ivorian Education System',
  description:
    '6-4-3 system: 6 years Primary, 4 years Collège, 3 years Lycée, 3+ years University',
  flag: '🇨🇮',
  phoneCode: '+225',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 10, // October
    academicYearEnd: 6, // June
    vacationPeriods: [
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'Christmas holiday break',
      },
      {
        name: 'Easter Break',
        startMonth: 3,
        endMonth: 4,
        description: 'Easter holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 7,
        endMonth: 9,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'École Primaire',
      description: '6 years of primary education',
      ageRange: [6, 11],
      order: 1,
      classLevels: [
        {
          id: 'cp1',
          name: 'CP 1',
          ageRange: [6, 7],
          description: 'Cours Préparatoire 1',
          order: 1,
          subjects: ['Français', 'Mathématiques', 'Éveil'],
        },
        {
          id: 'cp2',
          name: 'CP 2',
          ageRange: [7, 8],
          description: 'Cours Préparatoire 2',
          order: 2,
          subjects: ['Français', 'Mathématiques', 'Éveil'],
        },
        {
          id: 'ce1',
          name: 'CE 1',
          ageRange: [8, 9],
          description: 'Cours Élémentaire 1',
          order: 3,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'ce2',
          name: 'CE 2',
          ageRange: [9, 10],
          description: 'Cours Élémentaire 2',
          order: 4,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'cm1',
          name: 'CM 1',
          ageRange: [10, 11],
          description: 'Cours Moyen 1',
          order: 5,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'cm2',
          name: 'CM 2',
          ageRange: [11, 12],
          description: 'Cours Moyen 2',
          order: 6,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
      ],
      subjects: [
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Éducation Civique',
      ],
      nationalExam: "Certificat d'Études Primaires Élémentaires (CEPE)",
    },
    {
      id: 'college',
      name: 'Collège',
      description: '4 years of collège education',
      ageRange: [12, 15],
      order: 2,
      classLevels: [
        {
          id: 'sixieme',
          name: '6ème',
          ageRange: [12, 13],
          description: 'Sixième année',
          order: 1,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'cinquieme',
          name: '5ème',
          ageRange: [13, 14],
          description: 'Cinquième année',
          order: 2,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'quatrieme',
          name: '4ème',
          ageRange: [14, 15],
          description: 'Quatrième année',
          order: 3,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'troisieme',
          name: '3ème',
          ageRange: [15, 16],
          description: 'Troisième année',
          order: 4,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
      ],
      subjects: [
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Anglais',
        'Espagnol',
        'Allemand',
      ],
      nationalExam: "Brevet d'Études du Premier Cycle (BEPC)",
    },
    {
      id: 'lycee',
      name: 'Lycée',
      description: '3 years of lycée education',
      ageRange: [16, 18],
      order: 3,
      classLevels: [
        {
          id: 'seconde',
          name: 'Seconde',
          ageRange: [16, 17],
          description: 'Seconde année',
          order: 1,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'premiere',
          name: 'Première',
          ageRange: [17, 18],
          description: 'Première année',
          order: 2,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'terminale',
          name: 'Terminale',
          ageRange: [18, 19],
          description: 'Terminale année',
          order: 3,
          subjects: [
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
      ],
      subjects: [
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Anglais',
        'Philosophie',
      ],
      nationalExam: 'Baccalauréat',
    },
  ],
};

// Egyptian Education System
export const egyptianSystem: EducationSystem = {
  id: 'egypt',
  countryCode: 'EG',
  systemName: 'Egyptian Education System',
  description:
    '6-3-3 system: 6 years Primary, 3 years Preparatory, 3 years Secondary, 4+ years University',
  flag: '🇪🇬',
  phoneCode: '+20',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 9, // September
    academicYearEnd: 6, // June
    vacationPeriods: [
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'Christmas holiday break',
      },
      {
        name: 'Easter Break',
        startMonth: 3,
        endMonth: 4,
        description: 'Easter holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 7,
        endMonth: 8,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'Primary School',
      description: '6 years of primary education',
      ageRange: [6, 11],
      order: 1,
      classLevels: [
        {
          id: 'primary1',
          name: 'Primary 1',
          ageRange: [6, 7],
          description: 'First year of primary',
          order: 1,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'primary2',
          name: 'Primary 2',
          ageRange: [7, 8],
          description: 'Second year of primary',
          order: 2,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'primary3',
          name: 'Primary 3',
          ageRange: [8, 9],
          description: 'Third year of primary',
          order: 3,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'primary4',
          name: 'Primary 4',
          ageRange: [9, 10],
          description: 'Fourth year of primary',
          order: 4,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'primary5',
          name: 'Primary 5',
          ageRange: [10, 11],
          description: 'Fifth year of primary',
          order: 5,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'primary6',
          name: 'Primary 6',
          ageRange: [11, 12],
          description: 'Sixth year of primary',
          order: 6,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
      ],
      subjects: [
        'Arabic',
        'English',
        'Mathematics',
        'Science',
        'Social Studies',
        'Religious Education',
        'Art',
        'Music',
      ],
      nationalExam: 'Primary School Certificate',
    },
    {
      id: 'preparatory',
      name: 'Preparatory School',
      description: '3 years of preparatory education',
      ageRange: [12, 14],
      order: 2,
      classLevels: [
        {
          id: 'prep1',
          name: 'Preparatory 1',
          ageRange: [12, 13],
          description: 'First year of preparatory',
          order: 1,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'prep2',
          name: 'Preparatory 2',
          ageRange: [13, 14],
          description: 'Second year of preparatory',
          order: 2,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'prep3',
          name: 'Preparatory 3',
          ageRange: [14, 15],
          description: 'Third year of preparatory',
          order: 3,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
      ],
      subjects: [
        'Arabic',
        'English',
        'Mathematics',
        'Science',
        'Social Studies',
        'Religious Education',
        'Art',
        'Music',
      ],
      nationalExam: 'Preparatory School Certificate',
    },
    {
      id: 'secondary',
      name: 'Secondary School',
      description: '3 years of secondary education',
      ageRange: [15, 17],
      order: 3,
      classLevels: [
        {
          id: 'secondary1',
          name: 'Secondary 1',
          ageRange: [15, 16],
          description: 'First year of secondary',
          order: 1,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'secondary2',
          name: 'Secondary 2',
          ageRange: [16, 17],
          description: 'Second year of secondary',
          order: 2,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
        {
          id: 'secondary3',
          name: 'Secondary 3',
          ageRange: [17, 18],
          description: 'Third year of secondary',
          order: 3,
          subjects: [
            'Arabic',
            'English',
            'Mathematics',
            'Biology',
            'Chemistry',
            'Physics',
          ],
        },
      ],
      subjects: [
        'Arabic',
        'English',
        'Mathematics',
        'Biology',
        'Chemistry',
        'Physics',
        'History',
        'Geography',
        'Economics',
      ],
      nationalExam: 'General Secondary Education Certificate (Thanaweya Amma)',
    },
  ],
};

// Moroccan Education System
export const moroccanSystem: EducationSystem = {
  id: 'morocco',
  countryCode: 'MA',
  systemName: 'Moroccan Education System',
  description:
    '6-3-3 system: 6 years Primary, 3 years Collège, 3 years Lycée, 3+ years University',
  flag: '🇲🇦',
  phoneCode: '+212',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 9, // September
    academicYearEnd: 6, // June
    vacationPeriods: [
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'Christmas holiday break',
      },
      {
        name: 'Easter Break',
        startMonth: 3,
        endMonth: 4,
        description: 'Easter holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 7,
        endMonth: 8,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'École Primaire',
      description: '6 years of primary education',
      ageRange: [6, 11],
      order: 1,
      classLevels: [
        {
          id: 'cp1',
          name: 'CP 1',
          ageRange: [6, 7],
          description: 'Cours Préparatoire 1',
          order: 1,
          subjects: ['Arabe', 'Français', 'Mathématiques', 'Éveil'],
        },
        {
          id: 'cp2',
          name: 'CP 2',
          ageRange: [7, 8],
          description: 'Cours Préparatoire 2',
          order: 2,
          subjects: ['Arabe', 'Français', 'Mathématiques', 'Éveil'],
        },
        {
          id: 'ce1',
          name: 'CE 1',
          ageRange: [8, 9],
          description: 'Cours Élémentaire 1',
          order: 3,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'ce2',
          name: 'CE 2',
          ageRange: [9, 10],
          description: 'Cours Élémentaire 2',
          order: 4,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'cm1',
          name: 'CM 1',
          ageRange: [10, 11],
          description: 'Cours Moyen 1',
          order: 5,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
        {
          id: 'cm2',
          name: 'CM 2',
          ageRange: [11, 12],
          description: 'Cours Moyen 2',
          order: 6,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
          ],
        },
      ],
      subjects: [
        'Arabe',
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Éducation Islamique',
      ],
      nationalExam: "Certificat d'Études Primaires",
    },
    {
      id: 'college',
      name: 'Collège',
      description: '3 years of collège education',
      ageRange: [12, 14],
      order: 2,
      classLevels: [
        {
          id: 'sixieme',
          name: '6ème',
          ageRange: [12, 13],
          description: 'Sixième année',
          order: 1,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'cinquieme',
          name: '5ème',
          ageRange: [13, 14],
          description: 'Cinquième année',
          order: 2,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'quatrieme',
          name: '4ème',
          ageRange: [14, 15],
          description: 'Quatrième année',
          order: 3,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
      ],
      subjects: [
        'Arabe',
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Anglais',
        'Éducation Islamique',
      ],
      nationalExam: "Brevet d'Études du Premier Cycle (BEPC)",
    },
    {
      id: 'lycee',
      name: 'Lycée',
      description: '3 years of lycée education',
      ageRange: [15, 17],
      order: 3,
      classLevels: [
        {
          id: 'seconde',
          name: 'Seconde',
          ageRange: [15, 16],
          description: 'Seconde année',
          order: 1,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'premiere',
          name: 'Première',
          ageRange: [16, 17],
          description: 'Première année',
          order: 2,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
        {
          id: 'terminale',
          name: 'Terminale',
          ageRange: [17, 18],
          description: 'Terminale année',
          order: 3,
          subjects: [
            'Arabe',
            'Français',
            'Mathématiques',
            'Histoire-Géographie',
            'Sciences',
            'Anglais',
          ],
        },
      ],
      subjects: [
        'Arabe',
        'Français',
        'Mathématiques',
        'Histoire-Géographie',
        'Sciences',
        'Anglais',
        'Philosophie',
        'Éducation Islamique',
      ],
      nationalExam: 'Baccalauréat',
    },
  ],
};

// American Education System
export const americanSystem: EducationSystem = {
  id: 'usa',
  countryCode: 'US',
  systemName: 'American Education System',
  description:
    'K-12 system: Kindergarten through 12th Grade, 4+ years University',
  flag: '🇺🇸',
  phoneCode: '+1',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 8,
  },
  calendar: {
    academicYearStart: 8, // August
    academicYearEnd: 5, // May
    vacationPeriods: [
      {
        name: 'Thanksgiving Break',
        startMonth: 11,
        endMonth: 11,
        description: 'Thanksgiving holiday break',
      },
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'Christmas holiday break',
      },
      {
        name: 'Spring Break',
        startMonth: 3,
        endMonth: 3,
        description: 'Spring holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 6,
        endMonth: 7,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'elementary',
      name: 'Elementary School',
      description: '5 years of elementary education (K-5)',
      ageRange: [5, 10],
      order: 1,
      classLevels: [
        {
          id: 'kindergarten',
          name: 'Kindergarten',
          ageRange: [5, 6],
          description: 'Kindergarten year',
          order: 1,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade1',
          name: '1st Grade',
          ageRange: [6, 7],
          description: 'First grade',
          order: 2,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade2',
          name: '2nd Grade',
          ageRange: [7, 8],
          description: 'Second grade',
          order: 3,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade3',
          name: '3rd Grade',
          ageRange: [8, 9],
          description: 'Third grade',
          order: 4,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade4',
          name: '4th Grade',
          ageRange: [9, 10],
          description: 'Fourth grade',
          order: 5,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade5',
          name: '5th Grade',
          ageRange: [10, 11],
          description: 'Fifth grade',
          order: 6,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
      ],
      subjects: [
        'English Language Arts',
        'Mathematics',
        'Science',
        'Social Studies',
        'Physical Education',
        'Art',
        'Music',
      ],
    },
    {
      id: 'middle',
      name: 'Middle School',
      description: '3 years of middle school education (6-8)',
      ageRange: [11, 13],
      order: 2,
      classLevels: [
        {
          id: 'grade6',
          name: '6th Grade',
          ageRange: [11, 12],
          description: 'Sixth grade',
          order: 1,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade7',
          name: '7th Grade',
          ageRange: [12, 13],
          description: 'Seventh grade',
          order: 2,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade8',
          name: '8th Grade',
          ageRange: [13, 14],
          description: 'Eighth grade',
          order: 3,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
      ],
      subjects: [
        'English Language Arts',
        'Mathematics',
        'Science',
        'Social Studies',
        'Physical Education',
        'Art',
        'Music',
        'Foreign Language',
      ],
    },
    {
      id: 'high',
      name: 'High School',
      description: '4 years of high school education (9-12)',
      ageRange: [14, 17],
      order: 3,
      classLevels: [
        {
          id: 'grade9',
          name: '9th Grade (Freshman)',
          ageRange: [14, 15],
          description: 'Freshman year',
          order: 1,
          subjects: ['English', 'Mathematics', 'Science', 'Social Studies'],
        },
        {
          id: 'grade10',
          name: '10th Grade (Sophomore)',
          ageRange: [15, 16],
          description: 'Sophomore year',
          order: 2,
          subjects: ['English', 'Mathematics', 'Science', 'Social Studies'],
        },
        {
          id: 'grade11',
          name: '11th Grade (Junior)',
          ageRange: [16, 17],
          description: 'Junior year',
          order: 3,
          subjects: ['English', 'Mathematics', 'Science', 'Social Studies'],
        },
        {
          id: 'grade12',
          name: '12th Grade (Senior)',
          ageRange: [17, 18],
          description: 'Senior year',
          order: 4,
          subjects: ['English', 'Mathematics', 'Science', 'Social Studies'],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Science',
        'Social Studies',
        'Physical Education',
        'Art',
        'Music',
        'Foreign Language',
        'Electives',
      ],
      nationalExam: 'High School Diploma',
    },
  ],
};

// British Education System
export const britishSystem: EducationSystem = {
  id: 'uk',
  countryCode: 'GB',
  systemName: 'British Education System',
  description:
    '4-6-2-3 system: 4 years Primary, 6 years Secondary, 2 years A-Level, 3+ years University',
  flag: '🇬🇧',
  phoneCode: '+44',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 10,
  },
  calendar: {
    academicYearStart: 9, // September
    academicYearEnd: 7, // July
    vacationPeriods: [
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'Christmas holiday break',
      },
      {
        name: 'Easter Break',
        startMonth: 3,
        endMonth: 4,
        description: 'Easter holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 7,
        endMonth: 8,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'primary',
      name: 'Primary School',
      description: '6 years of primary education (Reception to Year 6)',
      ageRange: [4, 10],
      order: 1,
      classLevels: [
        {
          id: 'reception',
          name: 'Reception',
          ageRange: [4, 5],
          description: 'Reception year',
          order: 1,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'Art',
            'Design & Technology',
          ],
        },
        {
          id: 'year1',
          name: 'Year 1',
          ageRange: [5, 6],
          description: 'First year of primary',
          order: 2,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'Art',
            'Design & Technology',
          ],
        },
        {
          id: 'year2',
          name: 'Year 2',
          ageRange: [6, 7],
          description: 'Second year of primary',
          order: 3,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'Art',
            'Design & Technology',
          ],
        },
        {
          id: 'year3',
          name: 'Year 3',
          ageRange: [7, 8],
          description: 'Third year of primary',
          order: 4,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'Art',
            'Design & Technology',
          ],
        },
        {
          id: 'year4',
          name: 'Year 4',
          ageRange: [8, 9],
          description: 'Fourth year of primary',
          order: 5,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'Art',
            'Design & Technology',
          ],
        },
        {
          id: 'year5',
          name: 'Year 5',
          ageRange: [9, 10],
          description: 'Fifth year of primary',
          order: 6,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'Art',
            'Design & Technology',
          ],
        },
        {
          id: 'year6',
          name: 'Year 6',
          ageRange: [10, 11],
          description: 'Sixth year of primary',
          order: 7,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'Art',
            'Design & Technology',
          ],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Science',
        'Art',
        'Design & Technology',
        'History',
        'Geography',
        'Music',
        'Physical Education',
        'Computing',
      ],
      nationalExam: 'Key Stage 2 SATs',
    },
    {
      id: 'secondary',
      name: 'Secondary School',
      description: '5 years of secondary education (Year 7 to Year 11)',
      ageRange: [11, 15],
      order: 2,
      classLevels: [
        {
          id: 'year7',
          name: 'Year 7',
          ageRange: [11, 12],
          description: 'First year of secondary',
          order: 1,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'History',
            'Geography',
          ],
        },
        {
          id: 'year8',
          name: 'Year 8',
          ageRange: [12, 13],
          description: 'Second year of secondary',
          order: 2,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'History',
            'Geography',
          ],
        },
        {
          id: 'year9',
          name: 'Year 9',
          ageRange: [13, 14],
          description: 'Third year of secondary',
          order: 3,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'History',
            'Geography',
          ],
        },
        {
          id: 'year10',
          name: 'Year 10',
          ageRange: [14, 15],
          description: 'Fourth year of secondary',
          order: 4,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'History',
            'Geography',
          ],
        },
        {
          id: 'year11',
          name: 'Year 11',
          ageRange: [15, 16],
          description: 'Fifth year of secondary',
          order: 5,
          subjects: [
            'English',
            'Mathematics',
            'Science',
            'History',
            'Geography',
          ],
        },
      ],
      subjects: [
        'English',
        'Mathematics',
        'Science',
        'History',
        'Geography',
        'Art',
        'Music',
        'Physical Education',
        'Design & Technology',
        'Computing',
        'Modern Foreign Languages',
      ],
      nationalExam: 'General Certificate of Secondary Education (GCSE)',
    },
    {
      id: 'sixth_form',
      name: 'Sixth Form',
      description: '2 years of sixth form education (Year 12 to Year 13)',
      ageRange: [16, 17],
      order: 3,
      classLevels: [
        {
          id: 'year12',
          name: 'Year 12 (Lower Sixth)',
          ageRange: [16, 17],
          description: 'First year of sixth form',
          order: 1,
          subjects: ['A-Level Subjects'],
        },
        {
          id: 'year13',
          name: 'Year 13 (Upper Sixth)',
          ageRange: [17, 18],
          description: 'Second year of sixth form',
          order: 2,
          subjects: ['A-Level Subjects'],
        },
      ],
      subjects: ['A-Level Subjects'],
      nationalExam: 'General Certificate of Education Advanced Level (A-Level)',
    },
  ],
};

// Canadian Education System
export const canadianSystem: EducationSystem = {
  id: 'canada',
  countryCode: 'CA',
  systemName: 'Canadian Education System',
  description:
    'K-12 system: Kindergarten through 12th Grade, 3-4 years University',
  flag: '🇨🇦',
  phoneCode: '+1',
  features: {
    usesTerms: true,
    hasExams: true,
    supportsElectives: true,
    maxSubjectsPerLevel: 8,
  },
  calendar: {
    academicYearStart: 9, // September
    academicYearEnd: 6, // June
    vacationPeriods: [
      {
        name: 'Christmas Break',
        startMonth: 12,
        endMonth: 1,
        description: 'Christmas holiday break',
      },
      {
        name: 'March Break',
        startMonth: 3,
        endMonth: 3,
        description: 'March holiday break',
      },
      {
        name: 'Summer Break',
        startMonth: 7,
        endMonth: 8,
        description: 'Long summer vacation',
      },
    ],
  },
  levels: [
    {
      id: 'elementary',
      name: 'Elementary School',
      description: '8 years of elementary education (K-7)',
      ageRange: [5, 12],
      order: 1,
      classLevels: [
        {
          id: 'kindergarten',
          name: 'Kindergarten',
          ageRange: [5, 6],
          description: 'Kindergarten year',
          order: 1,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade1',
          name: 'Grade 1',
          ageRange: [6, 7],
          description: 'First grade',
          order: 2,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade2',
          name: 'Grade 2',
          ageRange: [7, 8],
          description: 'Second grade',
          order: 3,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade3',
          name: 'Grade 3',
          ageRange: [8, 9],
          description: 'Third grade',
          order: 4,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade4',
          name: 'Grade 4',
          ageRange: [9, 10],
          description: 'Fourth grade',
          order: 5,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade5',
          name: 'Grade 5',
          ageRange: [10, 11],
          description: 'Fifth grade',
          order: 6,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade6',
          name: 'Grade 6',
          ageRange: [11, 12],
          description: 'Sixth grade',
          order: 7,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade7',
          name: 'Grade 7',
          ageRange: [12, 13],
          description: 'Seventh grade',
          order: 8,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
      ],
      subjects: [
        'English Language Arts',
        'Mathematics',
        'Science',
        'Social Studies',
        'Physical Education',
        'Art',
        'Music',
        'French',
      ],
    },
    {
      id: 'secondary',
      name: 'Secondary School',
      description: '4 years of secondary education (8-12)',
      ageRange: [13, 16],
      order: 2,
      classLevels: [
        {
          id: 'grade8',
          name: 'Grade 8',
          ageRange: [13, 14],
          description: 'Eighth grade',
          order: 1,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade9',
          name: 'Grade 9',
          ageRange: [14, 15],
          description: 'Ninth grade',
          order: 2,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade10',
          name: 'Grade 10',
          ageRange: [15, 16],
          description: 'Tenth grade',
          order: 3,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade11',
          name: 'Grade 11',
          ageRange: [16, 17],
          description: 'Eleventh grade',
          order: 4,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
        {
          id: 'grade12',
          name: 'Grade 12',
          ageRange: [17, 18],
          description: 'Twelfth grade',
          order: 5,
          subjects: [
            'English Language Arts',
            'Mathematics',
            'Science',
            'Social Studies',
          ],
        },
      ],
      subjects: [
        'English Language Arts',
        'Mathematics',
        'Science',
        'Social Studies',
        'Physical Education',
        'Art',
        'Music',
        'French',
        'Electives',
      ],
      nationalExam: 'High School Diploma',
    },
  ],
};

// Export all education systems
export const allEducationSystems: EducationSystem[] = [
  nigerianSystem,
  ghanaianSystem,
  kenyanSystem,
  ugandanSystem,
  tanzanianSystem,
  southAfricanSystem,
  zimbabweanSystem,
  senegaleseSystem,
  ivorianSystem,
  egyptianSystem,
  moroccanSystem,
  americanSystem,
  britishSystem,
  canadianSystem,
];

// Export individual systems for easy access
export {
  nigerianSystem as nigeria,
  ghanaianSystem as ghana,
  kenyanSystem as kenya,
  ugandanSystem as uganda,
  tanzanianSystem as tanzania,
  southAfricanSystem as southAfrica,
  zimbabweanSystem as zimbabwe,
  senegaleseSystem as senegal,
  ivorianSystem as ivoryCoast,
  egyptianSystem as egypt,
  moroccanSystem as morocco,
  americanSystem as usa,
  britishSystem as uk,
  canadianSystem as canada,
};
