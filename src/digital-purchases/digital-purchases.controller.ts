...
export class DigitalPurchasesController {
  constructor(
    ...
    console.log('✅ DigitalPurchasesController initialized - routes should be registered');
    console.log('Routes: POST /digital-purchases, GET /digital-purchases/my-library, GET /digital-purchases/:id/view-pdf, etc.');
  }
  ...
  @Header('Access-Control-Allow-Origin', process.env.CORS_ORIGIN || 'http://localhost:3000')
  ...
  async downloadFile(
    ...
    const purchase: any = await this.digitalPurchasesService.getDownloadLink(purchaseId, studentId);
    ...
    const fileUrl = digitalFile.downloadUrl || digitalFile.imageKitUrl || digitalFile.storagePath;
    ...
    if (!finalFileUrl && digitalFile.storagePath) {
      ...
      if (service.convertFilePathToUrl) {
        ...
        if (digitalFile.storagePath.includes('uploads')) {
          ...
          const relativePath = digitalFile.storagePath.substring(uploadsIndex + 'uploads'.length);
    ...
    if (finalFileUrl.startsWith('http://') || finalFileUrl.startsWith('https://')) {
      ...
      try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${digitalFile.originalName || 'download'}"`);
    ...
    // Set headers for file download with CORS
    res.setHeader('Content-Type', digitalFile.mimeType || 'application/octet-stream');
    res.setHeader('Content-Disposition', `attachment; filename="${digitalFile.originalName || 'download'}"`);
    res.setHeader('Access-Control-Allow-Origin', process.env.CORS_ORIGIN || 'http://localhost:3000');
    res.setHeader('Access-Control-Allow-Credentials', 'true');
    res.setHeader('Access-Control-Expose-Headers', 'Content-Disposition, Content-Type');
  ...
  async viewPdf(
    ...
    console.log(`[VIEW-PDF] Request from user ${req.user.id} for purchase ${purchaseId}`);
    ...
    const isTestMode = process.env.PAYMENT_TEST_MODE === 'true' || 
                       (process.env.NODE_ENV !== 'production' && process.env.PAYMENT_TEST_MODE !== 'false');

    // Allow access if purchase is COMPLETED, or if in test mode allow PENDING purchases
    if (purchase.status !== 'COMPLETED' && !(isTestMode && purchase.status === 'PENDING')) {
      console.error(`[VIEW-PDF] Purchase ${purchaseId} not completed (status: ${purchase.status}, testMode: ${isTestMode})`);
    ...
    if (!isBuyer && !isBeneficiary) {
      console.error(`[VIEW-PDF] Unauthorized access: user ${req.user.id} is not buyer (${purchase.buyerId}) or beneficiary (${purchase.studentId})`);
      return res.status(403).json({ 
        message: 'You do not have permission to view this content. Only the buyer or the beneficiary student can access it.' 
      });
    }

    console.log(`[VIEW-PDF] Authorization granted: isBuyer=${isBuyer}, isBeneficiary=${isBeneficiary}`);

    // Get PDF file - check all files and filter for PDF
    const allFiles = purchase.content?.files || [];
    console.log(`[VIEW-PDF] All files in purchase:`, allFiles.map(f => ({
      id: f.id,
    ...
    if (!pdfFile) {
      ...
      console.error(`[VIEW-PDF] Available files:`, allFiles.map(f => ({
        name: f.originalName,
        mimeType: f.mimeType,
        fileType: f.fileType,
      })));
      return res.status(404).json({ 
        message: 'PDF file not found for this purchase. Please ensure the content has a PDF file uploaded.' 
      });
    }
    ...
    if (pdfFile.imageKitUrl) {
      ...
      if (pdfFile.storagePath.startsWith('http')) {
        ...
        if (uploadsIndex !== -1) {
          const relativePath = pdfFile.storagePath.substring(uploadsIndex + 'uploads'.length);
          const normalizedPath = relativePath.replace(/^[\\/]+/, '').replace(/\\/g, '/');
    ...
    if (!fileUrl) {
      console.error(`[VIEW-PDF] No file URL available for purchase ${purchaseId}`);
      return res.status(404).json({ message: 'File URL not available' });
    }

    console.log(`[VIEW-PDF] Streaming PDF from: ${fileUrl.substring(0, 100)}...`);
    ...
    if (fileUrl.startsWith('http://') || fileUrl.startsWith('https://')) {
      try {
        ...
        if (!fileResponse.ok) {
          console.error(`[VIEW-PDF] Failed to fetch PDF from external source: ${fileResponse.status} ${fileResponse.statusText}`);
          return res.status(fileResponse.status).json({
        ...
        res.setHeader('Content-Type', contentType);
        res.setHeader('Content-Disposition', `inline; filename="${pdfFile.originalName || 'document.pdf'}"`);
        res.setHeader('Access-Control-Allow-Origin', process.env.CORS_ORIGIN || 'http://localhost:3000');
        res.setHeader('Access-Control-Allow-Credentials', 'true');
